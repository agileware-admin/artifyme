...
const asaas_service_js_1 = require("../services/asaas.service.js");
...
const purchaseCreditsSchema = zod_1.z.object({
    package: zod_1.z.enum(['small', 'medium', 'large']),
    region: zod_1.z.enum(['BR', 'PT']),
});
...
router.post('/credits/purchase', async (req, res) => {
    try {
        ...
        // Create order record
        const order = await connection_js_1.prisma.order.create({
            data: {
                userId: user.id,
                type: 'credits',
                status: 'pending',
        ...
        if (region === 'BR') {
            ...
            paymentResult = await (0, asaas_service_js_1.createAsaasPayment)({
                orderId: order.id,
            ...
            paymentResult = await (0, stripe_service_js_1.createStripeCheckout)({
                orderId: order.id,
...
router.post('/subscribe', async (req, res) => {
    try {
        ...
        if (region === 'BR') {
            // Use Asaas for Brazil
            subscriptionResult = await (0, asaas_service_js_1.createAsaasSubscription)({
                userId: user.id,
                plan,
                billingCycle,
                amount,
            ...
            // Use Stripe for Portugal/Europe
            subscriptionResult = await (0, stripe_service_js_1.createStripeSubscription)({
                userId: user.id,
                plan,
                billingCycle,
                amount,
...
router.get('/orders', async (req, res) => {
    try {
        ...
        const [orders, total] = await Promise.all([
            ...
            connection_js_1.prisma.order.count({ where: { userId: user.id } }),
        ]);
        res.json({
            orders,
            pagination: {
